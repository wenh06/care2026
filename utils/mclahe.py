...
def tf_batch_histogram(values, value_range, axis, nbins=100, dtype=tf.int32, use_map=True):
    ...
    if use_map:
        ...
        hist = tf.histogram_fixed_width(
            values_shift, [0.0, tf.cast(num_batch, tf.float32)], nbins=num_batch * nbins, dtype=dtype
        )

    return tf.reshape(hist, tf.concat([batch_dim, [nbins]], 0))


def mclahe(x, kernel_size=None, clip_limit=0.01, nbins=128, adaptive_hist_range=False, use_gpu=False):
    """
    ...
    if kernel_size is None:
        kernel_size = tuple(max(1, s // 8) for s in x.shape)
    ...
    with tf.compat.v1.variable_scope("clahe") as scope:
        ...
        if adaptive_hist_range:
            ...
            tf_x_block_scaled = (tf_x_block - tf.reshape(tf_x_hist_min_sub, tf_hist_norm_slice_shape)) / tf.reshape(
                tf_x_hist_norm_sub, tf_hist_norm_slice_shape
            )
